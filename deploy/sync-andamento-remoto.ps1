# Sincroniza andamentos no banco via SSH (usa .env.local do servidor)
...
$remote = "set -e; cd $Dir && git pull origin master && npm exec tsx scripts/sync-andamento-canonico.ts"
