-- Espelha andamento (aba Análise) para observações — todos os usuários veem o mesmo texto

UPDATE registros SET observacoes = andamento
WHERE andamento IS NOT NULL AND andamento <> '';

UPDATE registros SET andamento = observacoes
WHERE (andamento IS NULL OR andamento = '')
  AND observacoes IS NOT NULL AND observacoes <> '';

-- Andamentos canônicos (print Tabela_1)
